...
from .models import Categories, Tags, Product, Review
...
class TagsListView(ListAPIView):
    ...
    """
    queryset = Tags.objects.only('name')
...
class CategoriesListView(ListAPIView):
    ...
    """
    queryset = Categories.objects.only('title').prefetch_related('image', 'subcategories')
...
class SalesListView(APIView):
    ...
    def get(self, request) -> Response:
        ...
        queryset = Product.objects.only('images', 'price', 'title', 'dateForm', 'dateTo', 'salePrice').filter(salePrice__gt=0)
